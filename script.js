document.addEventListener('DOMContentLoaded', () => {
        ...
        data.forEach(sectionData => {
            ...
            sectionData.groups.forEach(group => {
                ...
                if (group.phase || group.subgroup) {
                    const groupHeader = document.createElement(group.phase ? 'h3' : 'h4'); // h3 для фаз, h4 для подразделов
                    groupHeader.classList.add(group.phase ? 'phase-header' : 'subgroup-header');
                    groupHeader.textContent = group.phase || group.subgroup;
    ...
    function handleSearch() {
        ...
        if (firstMatch) {
            ...
            setTimeout(() => {
                if (currentHighlight === firstMatch) {
